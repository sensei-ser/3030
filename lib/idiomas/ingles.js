...
const smsRowner = () => { return `\`\`\`¡¡THIS COMMAND ONLY THE NUMBER OF THE BOT CAN USE!!\`\`\`` }
const smsOwner = () => { return `\`\`\`¡¡THIS COMMAND ONLY MY CREATOR CAN USE IT!!\`\`\`` }
const smsMods = () => { return `\`\`\`¡¡THIS COMMAND ONLY MODERATORS AND MY CREATOR CAN USE IT!!\`\`\`\`` }
const smsPremium = () => { return `\`\`\`¡¡THIS COMMAND IS ONLY AVAILABLE FOR PREMIUM USERS AND MY CREATOR(A)!! TO GET PREMIUM BUY A PASS USING #pass premium\`\`\`` }
