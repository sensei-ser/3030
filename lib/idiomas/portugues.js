...
const smsRowner = () => { return `\`\`\`¡¡ESTE COMANDO SOMENTE O NÚMERO DO BOT PODE USAR!!\`\`\`` }
const smsOwner = () => { return `\`\`\`¡¡SÓ MEU CRIADOR PODE USAR ESTE COMANDO!!\`\`\`` }
const smsMods = () => { return `\`\`\`¡¡SOMENTE MODERADORES E MEU CRIADOR PODEM USAR ESTE COMANDO!!\`\`\`\`` }
const smsPremium = () => { return `\`\`\`¡¡ESTE COMANDO SÓ ESTÁ DISPONÍVEL PARA USUÁRIOS PREMIUM E MEU CRIADOR!! PARA SE TORNAR PREMIUM COMPRE UM PASSE USANDO #pass premium\`\`\`` }
const smsGroup = () => { return `\`\`\`¡¡ESTE COMANDO SÓ PODE SER USADO EM GRUPOS!!\`\`\`` }
const smsPrivate = () => { return `\`\`\`¡¡ESTE COMANDO SÓ PODE SER USADO PRIVADAMENTE!!\`\`\`` }
const smsAdmin = () => { return `\`\`\`¡¡ESTE COMANDO É APENAS PARA ADMINISTRADORES!!\`\`\`` }
const smsBotAdmin = () => { return `\`\`\`¡¡PRECISO SER ADMINISTRADOR PARA QUE VOCÊ POSSA USAR ESTE COMANDO!!!\`\`\`` }
const smsUnreg = () => { return `\`\`\`¡¡Você precisa ser registrado (a) para usar este comando, escreva #verificar Registrar!!\`\`\`` }
const smsRestrict = () => { return `\`\`\`¡¡Este comando é restrito pelo meu criador!!!\`\`\`` }

//main.js
const smsWelcome = () => { return `*╭┈⊰* @subject *⊰┈ ✦*\n*┊✨ BEM-VINDO A)!!*\n┊💖 @user\n┊📄 *Leia a descrição do grupo*\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✦*\n${String.fromCharCode(8206).repeat(850)}\n@desc`}
const smsBye = () => { return '*╭┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*\n┊ *@user*\n┊ *NO Não era digno de estar aqui !!* 🌟\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*'}
const smsSpromote = () => { return '*@user Foi promovido como o administrador do grupo!!*'}
const smsSdemote = () => { return '*@user Foi degradado como administrador de um grupo!!*'}
const smsSdesc = () => { return '*Conheça a nova descrição do grupo:*\n\n@desc'}
const smsSsubject = () => { return '*O nome do grupo foi atualizado!!*\n@subject'}
const smsSicon = () => { return '*A foto do grupo foi atualizada!!*'}
const smsSrevoke = () => { return '*Conheça o novo link do grupo!!*\n*@revoke*'}

const methodCode1 = () => { return ` MÉTODO DE VINCULAÇÃO` }
const methodCode2 = () => { return ` COMO VOCÊ QUER SE CONECTAR?` }
const methodCode3 = () => { return ` Opção` }
const methodCode4 = () => { return ` Código QR` }
const methodCode5 = () => { return ` Código de 8 dígitos.` }
const methodCode6 = () => { return ` Escreva apenas o número de` }
const methodCode7 = () => { return ` a opção de conectar.` }
const methodCode8 = () => { return ` CONSELHO` }
const methodCode9 = () => { return ` Se você usa Termux, Replit, Linux ou Windows` }
const methodCode10 = () => { return ` Use estes comandos para execução direta:` }
const methodCode11 = (chalk) => { return ` NÚMEROS ALÉM DOS NÃO PERMITIDOS ${chalk.bold.greenBright("1")} O ${chalk.bold.greenBright("2")}, SEM LETRAS OU SÍMBOLOS ESPECIAIS.\n${chalk.bold.yellowBright(" DICA: COPIE O NÚMERO DA OPÇÃO E COLE NO CONSOLE.")}` }
const methodCode12 = () => { return ` Comece com o código QR` }
const methodCode13 = () => { return ` Comece com código de 8 dígitos` }
const methodCode14 = () => { return ` Inicialização padrão com opções` }
const phNumber = (chalk) => { return ` Definir arquivo ${chalk.bold.greenBright("config.js")} O número inserido não possui código de país. ${chalk.bold.yellowBright(" Exemplo: +593090909090")}` }
const phNumber2 = (chalk) => { return ` Por favor insira o número do WhatsApp.\n${chalk.bold.yellowBright(" DICA: Copie o número do WhatsApp e cole-o no console.")}\n${chalk.bold.yellowBright(" Exemplo: +593090909090")}\n${chalk.bold.magentaBright('---> ')}` }
const phNumber3 = () => { return ` Certifique-se de adicionar o código do país.` }
const pairingCode = () => { return ` CÓDIGO DE VINCULAÇÃO:` }
