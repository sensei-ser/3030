...
let handler = async (m, { conn, text, args, groupMetadata }) => {
    ...
    for (let i = 0; i < member.length; i++) {
        ...
        if (!args[0]) {
        return conn.reply(m.chat, `🤍 Используйте команду с параметрами:\n1. \`.неактивные лист\` чтобы пометить участников как выключенных\n2. \`.неактивный снести\` чтобы исключить участников из списка`, m)
    }
    ...
    return conn.reply(m.chat, `🤍Недопустимый вариант. Использует \`лист\` чтобы просмотреть неактивных участников или \`удалить\` чтобы удалить их.`, m)
}
